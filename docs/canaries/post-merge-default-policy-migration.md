...
Its named historical delivery is `01801a1b-58f9-480f-8cee-54ea2bbeabb2`.
The documentation builder started from clean commit
...
`sha256:7847d2e8086a5a510f45f8fe71ffd048387b35818cc123179f22387019917530`,
keeps `[validate, dependency-review, codeql]` for exact PR-head evidence and
...
also run on pushes to `main`. The
[earlier canary](post-merge-check-contract.md) preserves the previous full-list
policy and closure blocker. Its historical configuration statements do not
describe this prepared configuration.
...
4. `mill.yaml` read from the exact recorded `candidateCommit` is valid YAML,
   passes the configuration schema, and omits
   `propose.postMergeRequiredChecks`. Current checkout bytes cannot substitute
   for this proof. An explicitly configured full list is ineligible.
...
fails with `DELIVERY_AUTHORITY_DRIFT`. An unmerged PR cannot acquire the binding;
a changed merge tree requires fresh exact-tree validation.
...
permits `post_merge_verified` and closure. Main's skipped `dependency-review`
is outside the configured main subset, while all three checks remain required
...
binding/event, and required main check results. A later readback must demonstrate
continued use of the same binding. Missing evidence blocks a recovery claim;
do not edit stored provenance or replay push/open to manufacture eligibility.
...
provider readback. No tag, publication, GitHub Release, merge, support expansion,
or automated downgrade is authorized or claimed here. Downstream repositories
continue to build and test natively without Mill.
