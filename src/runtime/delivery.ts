...
function mergeMethod(
  observation: GitHubObservation,
  delivery: DeliveryRecord,
): "merge" | "squash" | "rebase" | "linear_tree_preserving" {
  ...
  if (commit.tree === delivery.candidateTree) {
    const linear = delivery.allowedMergeMethods.filter(
      (method) => method === "squash" || method === "rebase",
    );
    const onlyLinear = linear[0];
    return linear.length === 1 && onlyLinear !== undefined
      ? onlyLinear
      : "linear_tree_preserving";
  }
...
export async function finalizeDraftPr(input: {
  ...
  try {
    ...
    const method = mergeMethod(observation, delivery);
    const allowedMethod =
      method === "linear_tree_preserving"
        ? delivery.allowedMergeMethods.some(
            (allowed) => allowed === "squash" || allowed === "rebase",
          )
        : delivery.allowedMergeMethods.includes(method);
    if (!allowedMethod) {
      throw new MillError(
